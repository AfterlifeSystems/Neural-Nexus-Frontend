...
const AvatarLocationPicker = ({ value, onChange, heightClassName = 'h-56' }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="block text-xs text-white/60">
        Visitors count as here within {radiusMeters} m
        <input
          type="number"
          min={MINIMUM_GEOFENCE_RADIUS_METERS}
          max={MAXIMUM_GEOFENCE_RADIUS_METERS}
          value={value?.geofenceRadiusMeters ?? DEFAULT_GEOFENCE_RADIUS_METERS}
          onChange={(changeEvent) =>
            onChange({
              ...value,
              geofenceRadiusMeters: Number(changeEvent.target.value),
            })
          }
          className="mt-1 w-full rounded-md border border-white/10 bg-black/40 px-2 py-1.5 text-sm text-neutral-200 focus:outline-none focus:ring-2 focus:ring-amber-400/50"
        />
