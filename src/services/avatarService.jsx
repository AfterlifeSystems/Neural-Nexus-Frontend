    ...
    return collapseDuplicateIdentityDocuments(
      documentEntries
        ...
        .map((documentEntry) => ({
          ...
          isReferenceMedia: documentEntry.is_reference_media === true,
 ...
 */
/**
 * Read the avatar's standard set of conversation starters.
 * GET /avatar/{assistant_id}/conversation_starters
 *
 * The creator and, for a public avatar, anyone signed in. Returns the stored
 * record (`starters`, `generated_at`, `source`) or null when none is stored
 * yet; nothing is generated by this call.
 *
 * @param {string} assistantId The avatar.
 * @returns {Promise<Object|null>}
 */
export const getAvatarConversationStarters = async (assistantId) => {
  const response = await requestJson(
    `/avatar/${encodeURIComponent(assistantId)}/conversation_starters`
  );
  return response?.conversation_starters ?? null;
};

/**
 * Ask the messaging service to write a fresh standard set now (creator-only).
 * POST /avatar/{assistant_id}/conversation_starters
 *
 * @param {string} assistantId The avatar.
 * @returns {Promise<Object|null>} The record now stored, or null.
 */
export const regenerateAvatarConversationStarters = async (assistantId) => {
  const response = await requestJson(
    `/avatar/${encodeURIComponent(assistantId)}/conversation_starters`,
    { method: 'POST', body: {} }
  );
 ...
 *
 * Resolves to MPEG audio and `X-Voice-Kind` (`instant` | `professional` |
 * `standard`). A 409 `voice_not_ready` (no clone and no standard voice) is
 * thrown as an ApiError whose `detail` says so and whose `body.collected_seconds`
 * reports progress, so the caller can open the Voice panel. A successful
 * `standard` kind still means no clone has been added to this model. A 409
